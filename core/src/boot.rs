...
pub fn parse_boot_image(data: &[u8]) -> Result<KernelImage> {
    ...
    if kernel_size == 0 {
        bail!("boot image kernel size is zero");
    }
    ...
    let (page_size, kernel_load_addr, kernel_offset) = match header_version {
        0..=2 => {
            ...
            if page_size == 0 || !page_size.is_power_of_two() {
                bail!("invalid legacy boot page size");
            }
...
mod tests {
    ...
    fn rejects_unsupported_header_version() {
        assert!(parse_boot_image(&image(5, 4096, 1)).is_err());
    }

    #[test]
    fn rejects_truncated_kernel() {
        let mut data = image(2, 4096, 8);
