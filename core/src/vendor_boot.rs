...
fn read_u32(data: &[u8], offset: usize) -> Result<u32> {
    ...
    let bytes = data
        .get(offset..end)
        .ok_or_else(|| anyhow::anyhow!("truncated vendor boot header"))?;
...
fn align_up(value: usize, alignment: usize) -> Result<usize> {
    if alignment == 0 || !alignment.is_power_of_two() {
        bail!("invalid vendor boot page size");
    }
    value
...
fn range(data: &[u8], offset: usize, size: usize, label: &str) -> Result<Vec<u8>> {
    let end = offset
        .checked_add(size)
        .ok_or_else(|| anyhow::anyhow!("{label} range overflow"))?;
    if end > data.len() {
        bail!("{label} extends past vendor boot image");
    }
...
pub fn parse_vendor_boot_image(data: &[u8]) -> Result<VendorBootImage> {
    ...
    if header_version != 3 && header_version != 4 {
        bail!("unsupported vendor_boot header version: {header_version}");
    }
    let page_size = read_u32(data, 12)?;
    if page_size == 0 || !page_size.is_power_of_two() {
        bail!("invalid vendor_boot page size");
    }
    ...
    let minimum_header = if header_version == 4 {
        V4_HEADER_MIN
    } else {
        V3_HEADER_MIN
    };
    if header_size < minimum_header || header_size > data.len() {
        bail!("invalid vendor_boot header size");
    }
    let header_end = align_up(header_size, page_size as usize)?;
    let ramdisk_end = header_end
        .checked_add(vendor_ramdisk_size)
    ...
    let (table_offset, table_size, bootconfig_offset, bootconfig_size) = if header_version == 4 {
        ...
        if entry_size == 0 && entries != 0 {
            bail!("invalid vendor ramdisk table entry size");
        }
        let expected = entries
            .checked_mul(entry_size)
            .ok_or_else(|| anyhow::anyhow!("vendor ramdisk table overflow"))?;
        if expected > table_size {
            bail!("vendor ramdisk table is smaller than its entries");
        }
        let table_offset = align_up(
            dtb_offset
                .checked_add(dtb_size)
                .ok_or_else(|| anyhow::anyhow!("DTB offset overflow"))?,
            page_size as usize,
        )?;
        let config_offset = align_up(
            table_offset
                .checked_add(table_size)
                .ok_or_else(|| anyhow::anyhow!("table offset overflow"))?,
            page_size as usize,
        )?;
        (
            Some(table_offset as u64),
            Some(table_size as u64),
            Some(config_offset as u64),
            Some(config_size as u64),
        )
    } else {
        (None, None, None, None)
    };
    ...
    Ok(VendorBootImage {
        ...
        kernel_load_addr,
        ramdisk_load_addr,
        vendor_ramdisk_offset: header_end as u64,
        vendor_ramdisk_size: vendor_ramdisk_size as u64,
        dtb_offset: dtb_offset as u64,
...
mod tests {
    ...
    fn rejects_bad_magic() {
        assert!(parse_vendor_boot_image(b"bad").is_err());
    }
}
